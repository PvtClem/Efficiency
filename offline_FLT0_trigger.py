...
def trigger_FLT0(channel, dict_trigger_parameter):
    ...
    #print(f"index_t1_crossing: {index_t1_crossing}")
    ...
    if len(index_t1_crossing) == 0:
        return [], [], [], "NoT1" 
    killer = "Tquiet" # want to check if i’m killing events because all T1 crossings are in T_quiet



    if np.all(index_t1_crossing <= 100) & (len(index_t1_crossing) > 0):
        killer = "Idx100"
    ...
    if T1_indices:
        return T1_indices, T1_amplitudes, NC_values, killer 
    else:
        return [], [], [], killer # Do nothing if no valid T1 crossings were found # an event is killed only in this case
