...
See the GP300_log.docx for reference, where you can see only the trigger threshold
...
Currently, 1. is the case in the real experiment.
...
2nd argument: dict_trig_params_file (='dict_trig_params_fir.csv' for filter_mode='fir', and 'dict_trig_params_notch.csv' for filter_mode='notch')
3rd argument: directory, which specifies the directory where simulation data is analyzed.
All directories are listed in COREAS-AN_sim_data_directory.txt.
...
for event_number,run_number in events_list:
    ...
    for n in range(du_id.shape[0]):
        ...
        for row in dict_trig_params: trig_params = row
        trig_params = {k: int(v) for k, v in trig_params.items()}
        ###print("trig_params:", trig_params)
        
        T1_idxs_X, T1_amps_X, NC_vals_X = trigger_FLT0(tadc_trace_X, trig_params)
        T1_idxs_Y, T1_amps_Y, NC_vals_Y = trigger_FLT0(tadc_trace_Y, trig_params)
        T1_idxs_Z, T1_amps_Z, NC_vals_Z = trigger_FLT0(tadc_trace_Z, trig_params)
        
        ### Now only the trigger of the X & Y arms are discused
        if T1_idxs_X:
            for m in range(len(T1_idxs_X)):
                if NC_vals_X[m] >= trig_params['nc_min'] and NC_vals_X[m] <= trig_params['nc_max']:
        ...
        if T1_idxs_Y:
            for m in range(len(T1_idxs_Y)):
                if NC_vals_Y[m] >= trig_params['nc_min'] and NC_vals_Y[m] <= trig_params['nc_max']:
